/**
 * Chain Monitor API 调试脚本
 * 直接测试 Worker API 的所有接口
 *
 * 运行方式:
 *   node debug/test-all-api.ts
 *
 * 本地 Worker 需要先启动:
...
async function testMetric(chain: Chain, metric: Metric): Promise<TestResult> {
  ...
  const url = `${WORKER_URL}/api/blockchain-monitor/${endpointMap[metric]}?chain=${chain}`;
...
function printSummary(results: TestResult[]) {
  ...
  for (const chain of CHAINS) {
    ...
    for (const metric of METRICS) {
      ...
      const cache = result.data?.cached ? ' (cached)' : '';
      console.log(`  ${metric}: ${icon} ${result.statuscode}${cache}`);
    }
    console.log('');
  }
...
async function testAll() {
  console.log('🚀 开始测试 Chain Monitor API');
