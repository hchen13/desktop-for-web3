 ...
 * 处理链上监控 API 路由
 */
async function handleBlockchainMonitorRoutes(
  request: Request,
  cache: Cache | null,
): Promise<Response> {
  const url = new URL(request.url);
  ...
  if (
    (path === '/api/blockchain-monitor/metrics' ||
      path === '/api/blockchain-monitor/all-metrics') &&
    request.method === 'GET'
  ) {
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    (globalThis as any).ALCHEMY_API_KEY = env.ALCHEMY_API_KEY;
