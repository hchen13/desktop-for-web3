...
### 文档安全 (⚠️ 重要)
**绝对禁止在任何文档、注释、提交信息中记录完整的敏感信息：**
- ❌ 完整 API Key
- ❌ 完整密码或 Token
- ❌ 完整密钥字符串

**如需引用敏感信息作为标识：**
- ✅ 只记录前 6-8 位字符，如 `2d703765...`
- ✅ 使用描述性名称，如 "Z.AI API Key"
- ✅ 使用环境变量占位符，如 `$Z_AI_API_KEY`

**Git 历史安全：**
- 敏感文件必须在首次提交前加入 `.gitignore`
- 如不慎提交敏感信息，立即使用 `git filter-branch` 重写历史
- 强制推送前务必通知团队成员
- 无法撤销的 API Key 尤其危险，需格外谨慎
